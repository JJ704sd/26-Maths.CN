...
允许直接使用队友结果和独立运行记录，按实际路径建立结论与证据对应关系，不强制固定报告文件名。2026 国赛先读取 [专用规则](../5writing/references/cumcm-2026.md)，按最新适用要求检查模板、匿名性、AI 声明与详情文件。不能只凭模板原样保留就判合规。

逐项检查模型公式与代码实现、单位和结果版本；未独立重跑的结果标为未复现。队友结果更新后复核受影响的正文、图表和摘要。参考文献必须核实真实性及对论断的支持，不以引用数量判质量。
...
脚本只扫描文本，不生成论文，也不编译 PDF。脚本结果是诊断线索；先确认是否真实缺陷，不能为消除误报而删除必要的 AI 披露、附录路径或支撑文件清单。真实缺陷修复后重跑。
...
- `\includegraphics{}` 引用的图片文件是否真实存在。路径相对于实际编译工作目录；本地默认从论文入口目录编译。
- `\caption{}` 是否存在。
- 数据图是否放在对应结果/分析章节。
...
先将工作目录切换到论文入口所在目录，再用入口文件名编译，避免章节和图片路径随调用位置改变。
...
每项状态为 PASS、FAIL、NOT_VERIFIED 或有理由的 N/A。总体状态优先级：存在已确认硬错误则 FAIL；无硬错误但必要检查未完成则 NOT_VERIFIED；所有适用必要检查完成且通过才 PASS。编译、逐页视觉检查、核心结果复现或适用的 AI 提交材料检查缺失，不能以“已说明原因”替代通过。PASS 仅表示本次范围内检查通过，不保证评奖或检测结果。
...
- 论文入口引用的章节文件不存在。
- 入口未包含必要正文；单文件论文不因缺少 include/input 被判失败。
- 正文章节缺少一级标题（Typst `= ` 后缺空格，LaTeX `\section{}` 缺失）。
- 章节顺序明显错误或重复。
- 正文仍有占位符。
- 正文误混入内部工作指令或身份信息；合规的 AI 披露和附录文件清单不属于泄露。
- 引用的图片不存在。
- 关键数值与结果记录冲突。
- 伪造引用、核心外部论断缺少必要来源，或适用的 AI 声明与实际使用不符。
- 编译器可用但论文编译失败。
...
- 某章节过短或明显不均衡。
- caption 偏长。
- 非核心背景说明的来源支持较弱（逐条指出，不设文献数量门槛）。
- 图表后解释文字不足。
- 视觉检查工具不可用：该项及总体不能判 PASS，记录 NOT_VERIFIED。
- 完整复现未完成：列出已运行范围和未验证结论，总体不能判 PASS。
